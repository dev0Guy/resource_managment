...
class ClusterMetricRenderer:
    def __init__(
        ...
        self.status_colors = {
            JobStatus.Running: (0, 0, 255),  # Blue
            JobStatus.Completed: (0, 0, 0),  # Black
            JobStatus.Failed: (255, 0, 0),  # Red
            JobStatus.Pending: (0, 255, 0),  # Green
            JobStatus.NotCreated: (128, 128, 128),  # Gray
        }
        ...
        self.status_order = {
            ...
            JobStatus.NotCreated: 4,
        }

    def render(
        self,
        new_info: MetricResourceManagementInformation,
        new_observation: MetricResourceManagementObservation,
        ...
        jobs_section_height = self._calculate_jobs_section_height(
            new_observation["jobs"], new_observation["status"]
        )

        # Draw jobs section at the top (below tick counter)
        self._draw_jobs(
            new_observation["jobs"],
            new_observation["status"],
            new_observation["arrival"],
        )
    ...
    def _draw_tick_counter(self, info: MetricResourceManagementInformation) -> None:
        """
    ...
    def _draw_single_job(
        self, job: npt.NDArray[float], status: JobStatus, x: int, y: int
    ) -> None:
        """
        ...
        border_width = 3  # Thicker border to show status clearly
